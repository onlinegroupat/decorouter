...
(function (factory) {
    ...
    __decorate([
        index_1.route('posts/:id'),
        __param(0, index_1.routeParam)
    ], Example.prototype, "post", null);
    Example = __decorate([
        index_1.route('/')
    ], Example);
    let example = new Example();
    document.getElementById('test1').onclick = (e) => {
        e.preventDefault();
        example.test1();
        return false;
    };
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (factory) {
    ...
    class RouterImpl {
        ...
        addRouteMethod(target, methodName, path) {
            this.methods.push({ target, methodName, path });
        }
        addRouteObject(routeObject, path) {
            for (let entry of this.methods) {
                if (routeObject instanceof entry.target) {
                    let fullPath = path + entry.path;
                    let route = new Route(fullPath);
                    this.routes.set(route, (params) => {
                        let handler = routeObject[entry.methodName];
                        handler.apply(routeObject, params);
                    });
                }
            }
        }
        init() {
            console.log('RouterImpl::init');
        }
    }
    function routeMethod(path) {
        return (target, propertyKey, descriptor) => {
            console.log("route decorator called with ", target, propertyKey, path, descriptor);
            routerImpl.addRouteMethod(target, propertyKey, path);
            return descriptor;
        };
    }
    ...
    function routeClass(path) {
        return (target) => {
            return class Wrapper extends target {
                constructor() {
                    super();
                    routerImpl.addRouteObject(this, path);
                }
            };
        };
    }
    exports.routeClass = routeClass;
    function routeParam(target, key, index) {
        console.log('routeParam called', target, key, index);
    }
